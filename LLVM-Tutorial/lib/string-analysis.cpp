...
namespace
{
	...
	bool isCString(const llvm::GlobalVariable& gv) noexcept
	{
		...
		{
			PASS_DEBUG(gv.getNameOrAsOperand() 
				<< " is either mutable or does not have an initializer.\n");
...
jvs::StringAnalyzer::Result jvs::StringAnalyzer::run(llvm::Module& m)
{
	...
	for (llvm::GlobalVariable& gv : llvm::make_filter_range(m.globals(), isCString))
	{
		...
		auto stringVal =
			llvm::cast<llvm::ConstantDataArray>(gv.getInitializer())->getAsCString();
...
// string analyzer analysis pass
jvs::StringAnalyzer::Result jvs::StringAnalyzer::run(llvm::Module& m, llvm::ModuleAnalysisManager& /*unused*/)
{
	return run(m);
}
...
void jvs::StringAnalyzer::registerAnalysis(llvm::ModuleAnalysisManager& an)
{
	an.registerPass([] {return llvm::PassInstrumentationAnalysis{}; });

	an.registerPass([] {return StringAnalyzer{}; });
}


// string analyzer printing pass
jvs::StringAnalyzerPrinter::StringAnalyzerPrinter(llvm::raw_ostream& os) : os_(os)
{
...
llvm::PreservedAnalyses  jvs::StringAnalyzerPrinter::run(llvm::Module& m, llvm::ModuleAnalysisManager& am)
{
...
bool jvs::StringAnalyzerPrinter::registerPipelinePass(llvm::StringRef name, llvm::ModulePassManager& mpm,
